...
func _ready() -> void:
	...
	if !b_title_ready:
		...
		type_icon.custom_minimum_size.x = 50#type_icon.size.y
...
func configure(depend: int, path: String, type: StringName):
	...
	if !FileAccess.file_exists(path):
		get_theme_stylebox('panel', 'GraphNode').bg_color = Color.INDIAN_RED
		get_theme_stylebox('panel_selected', 'GraphNode').bg_color = Color.INDIAN_RED
