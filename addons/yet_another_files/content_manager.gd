...
var old_fs_path := 'res://'
var icon_size: EIconSize = EIconSize.MEDIUM
...
func _ready() -> void:
	...
	var tree = %Hierarchy/Scroll/VBox/Tree_Project as Tree
...
func _on_but_settings_ready() -> void:
	...
	for i in EIconSize.size():
		...
		icon_size_popup.set_item_disabled(i, !is_instance_valid(CONTENT_ITEM[i]))
	...
	change_icon_size(icon_size, false)
...
func change_icon_size(new: EIconSize, save_cfg := true):
	icon_size = new
	...
	update_content()
	if save_cfg:
		plugin.yafcfg_set('Display:icon_size', icon_size)
		plugin.yafcfg_flush()
...
func update_status(field: StringName):
	match field:
		&'elem_selected':
			%StatusBar/ElemSelected.text = '{elem} elements | {selected} selected'.format({
